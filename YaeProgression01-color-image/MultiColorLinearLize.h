...
Mat MultiColorLinearLize(Mat original_image, Mat inpaint_domain, double lambda, double theta,
	double tolerant, int max_gauss_seidel, int multi_coarse, int multi_mid, int multi_fine, int max_step, int current_step)
{
	...
	if (current_step < max_step) {
		...
		half_result = MultiColorLinearLize(half_image, half_domain, lambda, theta, tolerant, max_gauss_seidel, multi_coarse, multi_mid, multi_fine, max_step, current_step + 1);
	...
	return LinearLizeColorInpaint(toinpaint_image, inpaint_domain, lambda, theta, tolerant, max_gauss_seidel, max_iteration);
}